...
export default function Dialogue({ role }: Props) {
  ...
  return (
    <div className="min-h-[50vh] flex flex-col bg-white rounded-2xl sm:rounded-3xl border border-black/5 shadow-sm overflow-hidden flex-1">
      {/* Header */}
      <div className="p-4 sm:p-6 border-b border-black/5 flex justify-between items-center gap-3 bg-[#F5F2ED]/30 shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 bg-[#141414] text-white rounded-full flex items-center justify-center shrink-0">
            {role === 'RESIDENT' ? <User size={20} /> : <Bot size={20} />}
          </div>
          <div className="min-w-0">
            <h3 className="font-medium text-sm sm:text-base truncate">{role === 'RESIDENT' ? 'Architect Sarah' : 'Resident John'}</h3>
            <p className="text-[10px] uppercase tracking-widest opacity-40">Active Consultation - Sector A</p>
          </div>
        </div>
        <button className="text-xs font-bold uppercase tracking-widest text-[#A3B18A] hover:underline shrink-0 touch-manipulation py-1">View Case File</button>
      </div>

      {/* Messages Area */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6 bg-[#F5F2ED]/10 scroll-smooth min-h-0">
        {messages.map((msg) => (
          <motion.div 
          ...
          >
            <div className={`max-w-[85%] sm:max-w-[70%] space-y-1 ${msg.sender === role ? 'items-end' : 'items-start'}`}>
      ...
      <div className="p-4 sm:p-6 border-t border-black/5 bg-white shrink-0">
        <div className="flex gap-3 sm:gap-4">
          <input 
            ...
            className="flex-1 min-w-0 bg-[#F5F2ED] border-none rounded-xl px-4 sm:px-6 py-3 sm:py-4 text-sm outline-none focus:ring-2 ring-[#A3B18A]/20 disabled:opacity-50"
          />
          <button 
            onClick={handleSend}
            disabled={isTyping || !input.trim()}
            className="bg-[#A3B18A] text-[#141414] p-3 sm:p-4 rounded-xl hover:bg-[#92a078] transition-colors disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation shrink-0"
          >
